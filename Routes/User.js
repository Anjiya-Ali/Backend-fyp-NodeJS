...
router.post('/LoginUser', [
    ...
    try {
        ...
        if (role === 'Student') {
            ...
            return res.json({ success, role: 'Student', authtoken, id: user.id, name: name });

        }
        else if (role === 'Teacher') {
            const name = user.first_name + " " + user.last_name;
            return res.json({ success, role: 'Teacher', authtoken, id: user.id, name: name });
        }
    }
...
router.get('/GetProfilePictureOther/:profileId', fetchuser, async (req, res) => {
    const userId = req.user.id;
    const profileId = req.params.profileId;
    const ObjectId = mongoose.Types.ObjectId;
