...
router.post('/GetAllScheduledMeeting', fetchuser, async (req, res) => {
    ...
    try {
        ...
        requestedISODate.setDate(requestedISODate.getDate());
        requestedISODate.setUTCHours(0, 0, 0, 0);

        const nextDay = new Date(requestedISODate);
        nextDay.setDate(nextDay.getDate() + 1);

        console.log(requestedISODate)
        console.log(nextDay)
        const scheduled_meetings = await Meetings.find({ 
            user_id: user_id,
            date: {
                $gte: requestedISODate,
                $lt: nextDay
            }
        });
        ...
        console.log(scheduled_meetings)
